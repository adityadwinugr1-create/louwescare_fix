                    ...
                    <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="mt-6 flex justify-end gap-3 pt-4 border-t border-gray-100">
                            ...
                            <template x-if="remainingBill > 0">
                                <button type="button" @click="if(document.querySelectorAll('input[name=\'selected_items[]\']:checked').length === 0) { alert('Harap checklist item terlebih dahulu!'); return; } window.autoUpdateStatus(); openPaymentModal()" class="inline-flex items-center px-6 py-3 bg-green-600 border border-transparent rounded-lg font-bold text-sm text-white hover:bg-green-700 shadow-md transition">Bayar</button>
                            </template>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function populateInvoice(data) {
            ...
            Object.values(groupedItems).forEach(group => {
                ...
                // LOGIKA CENTANG MUNCUL DI SINI (HTML)
                let estContent = estStr;
                if (group.diambil_count > 0) {
                    // Gunakan kode HTML &#10004; agar logo centang aman saat dicetak di printer thermal
